...
export function SearchForm({
  ...
  const handleChange = (e: ChangeEvent<HTMLInputElement>) => {
    const rn = encodeURIComponent(e.target.value);
    setRn(rn);
    setV(e.target.value);
  };
  ...
  return (
    <form
      className={cn(
        "flex border-muted-foreground/60 border-2 rounded",
    ...
    >
      <input
        ...
        type="search"
        className={cn(
          "h-8 py-2 pl-1 text-base text-tertiary order-1",
      ...
      <Button
        ...
        className={cn(
          ...
          "focus-visible:outline focus-visible:outline-2 focus-visible:outline-ring/60",
      ...
      >
        <div>
          <Search strokeWidth={2.4} aria-hidden="true" />
        </div>
      </Button>
    </form>
  );
