  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="flex flex-col px-1">
        <DropdownMenuItem className={`pl-1 pr-0 py-1 flex justify-between ${lang === 'en' ? 'order-last' : ''}`}>
        <Link href={redirectedPathname('en')} className="flex items-center justify-between w-[126px]">
          <p>{t.english}</p>
          <div>
        ...
        <Link href={redirectedPathname('ru')} className="flex items-center justify-between w-[126px]">
          <p>{t.russian}</p>
          <div>
