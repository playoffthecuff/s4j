...
export default function DateTimeBlock({
  ...
  return (
    <div className={className}>
      <time dateTime={`${year}-${(''+(month+1)).padStart(2,'0')}-${("" + date).padStart(2, "0")}`}>
        <div className="flex gap-2 items-end">
          <p className="text-[42px] leading-none font-mono">
            {("" + date).padStart(2, "0")}
          </p>
          <div className={`${lng === 'en' && 'order-first'} w-full`}>
            {past && <p className="font-mono text-muted-foreground font-bold leading-tight"></p>}
            <p className={`text-base leading-tight mb-[2px] ${lng === "en" && "order-first"}`}>
              {m[lng][month]}
            </p>
          </div>
        </div>
        <div className="flex gap-2 text-base mt-1">
          ...
          <p className={`text-muted-foreground ${lng === 'en' && 'order-first'} ${past && 'w-full'}`}>{l[lng][day]}</p>
        </div>
      </time>
    </div>
  );
