import { fetchGalleryImage } from "@/app/[lang]/fetchImage";
import { Gallery } from "@/components/gallery";
import { Locale } from "@/i18n-config";
import { Metadata } from "next";
import { notFound } from "next/navigation";
import { fetchSearchedGalleryImageSlugs } from "./fetchSearchedImageSlugs";
...
export async function generateMetadata(props: {
  ...
  return {
    alternates: {
      canonical:
        params.lang === "en"
          ? `/en/search/gallery/${params.slug}`
          : `/ru/search/gallery/${params.slug}`,
      languages: {
        en: `/en/search/gallery/${params.slug}`,
        ru: `/ru/search/gallery/${params.slug}`,
...
export default async function Page(props: {
  params: Promise<{ lang: Locale; slug: string; q: string }>;
}) {
  const params = await props.params;
  const query = decodeURIComponent(params.q);
  const slugs = await fetchSearchedGalleryImageSlugs(params.lang, query);
